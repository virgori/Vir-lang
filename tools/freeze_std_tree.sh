...
copy_tree() {
  ...
  if command -v rsync >/dev/null 2>&1; then
    rsync -a --delete --exclude '.git/' --exclude '__pycache__/' --exclude '*.pyc' --exclude '.DS_Store' "$src"/ "$dst"/
  else
    rm -rf "$dst"
    mkdir -p "$dst"
    cp -a "$src"/. "$dst"/
...
set_source_version() {
  ...
  sed -E \
    ...
    "$src" >"$tmp"
  mv "$tmp" "$src"
  [ "$(source_version "$src")" = "$version" ] || die "failed to stamp compiler source version v$version: $src"
}

build_freeze_bin() {
  local dest="$1" version="$2" seed src output smoke_src smoke_bin banner
  seed="${VIRC:-bin/virc}"
  if [ ! -x "$seed" ]; then seed="dist/virc-next"; fi
...
EOF
  ...
  if command -v codesign >/dev/null 2>&1; then codesign -f -s - "$smoke_bin" >/dev/null 2>&1 || true; fi
  [ "$("$smoke_bin" 2>&1 | tr '\n' ' ' | sed -E 's/[[:space:]]+$//')" = "30 90" ] || die "--with-bin: frozen compiler smoke test failed"
  rm -f "$smoke_src" "$smoke_bin"
  echo "✓ bin/virc v$version ($(stat -f%z "$output" 2>/dev/null || stat -c%s "$output") bytes, built + smoke-tested)"
}

cmd_list() {
  if [ ! -d "$ROOT_FREEZE" ]; then echo "(no freezes yet under $ROOT_FREEZE/)"; return 0; fi
  find "$ROOT_FREEZE" -mindepth 2 -maxdepth 2 -type d | LC_ALL=C sort | while read -r d; do
    if [ -f "$d/MANIFEST.json" ]; then echo "$d"; fi
  done
}
...
do_freeze() {
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --version) [ $# -ge 2 ] || die "--version needs a semver"; requested_version="${2#v}"; shift ;;
      --readonly) readonly=1 ;;
      --force) force=1 ;;
      -h|--help) usage; exit 0 ;;
  ...
  case "$kind" in
    release) [[ "$name" =~ ^v?[0-9]+\.[0-9]+(\.[0-9]+)?([.-].*)?$ ]] || die "release name should look like v2.2.0 (got: $name)" ;;
    experimental) [[ "$name" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] || die "experimental slug must be [A-Za-z0-9._-]+ (got: $name)" ;;
  ...
  if [ "$kind" = release ]; then
    ...
    if [ -n "$requested_version" ] && [ "$requested_version" != "$compiler_version" ]; then die "--version v$requested_version does not match release $name"; fi
  ...
  [[ "$compiler_version" =~ ^[0-9]+\.[0-9]+\.[0-9]+([.-].*)?$ ]] || die "compiler version must be semver-like (got: $compiler_version)"

  local dest="$ROOT_FREEZE/$kind/$name"
  if [ -e "$dest" ] && [ "$force" -ne 1 ]; then die "exists: $dest (pass --force to replace)"; fi
  if [ -e "$dest" ]; then chmod -R u+w "$dest" 2>/dev/null || true; fi
  rm -rf "$dest"
  mkdir -p "$dest"
  ...
  if [ -d "$std_src/vir/compiler" ]; then copy_tree "$std_src/vir/compiler" "$dest/compiler_src/stdlib_vir_compiler"; echo "✓ compiler_src/stdlib_vir_compiler/"; fi
  for f in virc_stage1.vri virc_boot.vri; do
    if [ -f "$f" ]; then cp -a "$f" "$dest/compiler_src/$f"; echo "✓ compiler_src/$f"; fi
  done

  mkdir -p "$dest/bin"
  if [ "$with_bin" -eq 1 ]; then build_freeze_bin "$dest" "$compiler_version"; fi
  ...
  if [ "$with_stage1" -eq 1 ]; then
    ...
    mkdir -p "$dest/stage1"; cp -a virc_stage1.vri "$dest/stage1/"
    if [ -x dist/virc-stage1 ]; then cp -a dist/virc-stage1 "$dest/stage1/virc-stage1"; fi
    echo "✓ stage1/"
  fi
...
EOF
  ...
  if [ -f "$std_src/../BOOTSTRAP_REPORT.md" ]; then cp -a "$std_src/../BOOTSTRAP_REPORT.md" "$dest/BOOTSTRAP_REPORT.md"; echo "✓ BOOTSTRAP_REPORT.md"
  elif [ -f BOOTSTRAP_REPORT.md ]; then cp -a BOOTSTRAP_REPORT.md "$dest/BOOTSTRAP_REPORT.md"; echo "✓ BOOTSTRAP_REPORT.md"; fi

  write_manifest "$dest" "$kind" "$name" "$compiler_version"
  if [ "$readonly" -eq 1 ]; then chmod -R a-w "$dest"; echo "✓ marked read-only"; fi
  echo ""
  echo "FREEZE_OK $dest"
}
...
main() {
  ...
  case "$1" in
    -h|--help) usage; exit 0 ;;
    list) cmd_list ;;
    verify) [ $# -ge 2 ] || die "verify needs a path"; cmd_verify "$2" ;;
    release|experimental) [ $# -ge 2 ] || die "$1 needs a name"; local kind="$1" name="$2"; shift 2; do_freeze "$kind" "$name" "$@" ;;
