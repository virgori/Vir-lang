...
typedef enum {
    ...
    /* Network */
    BUILTIN_NET_LISTEN,    /* net_listen(port) → slot */
    BUILTIN_NET_ACCEPT,    /* net_accept(slot) → client_slot */
    BUILTIN_NET_RECV_STR,  /* net_recv(slot) → string */
    BUILTIN_NET_SEND_STR,  /* net_send(slot, str) */
    BUILTIN_NET_CLOSE,     /* net_close(slot) */
    BUILTIN_NET_SEND_ALL,  /* net_send_all(slot, str) loop */
    BUILTIN_NET_SET_NODELAY,/* net_set_nodelay(slot, flag) */
    /* Extended string */
    BUILTIN_STR_FIND,      /* str_find(s, sub) → index */
    BUILTIN_STR_SLICE,     /* str_slice(s, start, end) */
    BUILTIN_STR_STARTS,    /* str_starts_with(s, pfx) */
    BUILTIN_STR_CONTAINS,  /* str_contains(s, sub) */
    BUILTIN_STR_SPLIT,     /* str_split(s, delim) → arr */
    BUILTIN_STR_UPPER,     /* str_upper(s) */
    BUILTIN_STR_LOWER,     /* str_lower(s) */
    BUILTIN_STR_REPLACE,   /* str_replace(s, old, new) */
    BUILTIN_STR_TRIM,      /* str_trim(s) */
    BUILTIN_STR_ALLOC,     /* str_alloc(size) */
    BUILTIN_SPRINTF,       /* sprintf(fmt, arg) */
    /* Type conversion / Logic */
    BUILTIN_NOT,           /* not(x) / logical_not(x) */
    BUILTIN_I_TO_F,        /* i_to_f(n) → double bits */
    BUILTIN_F_TO_I,        /* f_to_i(d) → int */
    BUILTIN_F_TO_STR,      /* f_to_str(d) → string */
    BUILTIN_STR_TO_F,      /* str_to_f(s) → double bits */
    /* Syscall / OS */
    BUILTIN_TIME,          /* time_ms() → milliseconds */
    BUILTIN_ENV_GET,       /* env_get(name) → string */
    BUILTIN_SLEEP,         /* sleep_ms(ms) */
    BUILTIN_ADDR,          /* addr(x) → pseudo-pointer */
    BUILTIN_DEREF,         /* deref(p) → load pseudo-pointer */
} builtin_id_t;
