...
typedef enum {
    ...
    Q_STR_SLICE     = 0x74,   /* dest = str[src1_start..src2_end] */
    Q_STR_FIND      = 0x75,   /* dest = strpos(src1,src2) or -1   */
    Q_NOT           = 0x76,   /* dest = !src1                  */
    ...
    /* Network I/O */
    Q_NET_LISTEN    = 0xC0,   /* dest = net_listen(port)       */
    Q_NET_ACCEPT    = 0xC1,   /* dest = net_accept(srv_slot)   */
    Q_NET_RECV_STR  = 0xC2,   /* dest = net_recv_str(slot)     */
    Q_NET_SEND_STR  = 0xC3,   /* net_send_str(slot, str)       */
    Q_NET_CLOSE     = 0xC4,   /* net_close(slot)               */
    Q_NET_SEND_ALL  = 0xC5,   /* net_send_all(slot, str) loop  */
    Q_NET_SET_NODELAY = 0xC6, /* net_set_nodelay(slot, flag)   */

    /* Extended string operations */
    Q_STR_STARTS    = 0xD2,   /* dest = str_starts_with(s,pfx) */
    Q_STR_CONTAINS  = 0xD3,   /* dest = str_contains(s, sub)   */
    Q_STR_SPLIT     = 0xD4,   /* dest = str_split(s, delim)arr */
    Q_STR_UPPER     = 0xD5,   /* dest = str_upper(s)           */
    Q_STR_LOWER     = 0xD6,   /* dest = str_lower(s)           */
    Q_STR_REPLACE   = 0xD7,   /* dest = str_replace(s,old,new) */
    Q_STR_TRIM      = 0xD8,   /* dest = str_trim(s)            */
    Q_STR_ALLOC     = 0xD9,   /* dest = str_alloc(size)        */
    Q_SPRINTF       = 0xDA,   /* dest = sprintf(fmt, a, b)     */

    /* Type conversion */
    Q_I_TO_F        = 0xE1,   /* dest = (double)src1           */
    Q_F_TO_I        = 0xE2,   /* dest = (int64)src1            */
    Q_F_TO_STR      = 0xE3,   /* dest = ftoa(src1)             */
    Q_STR_TO_F      = 0xE4,   /* dest = atof(src1)             */

    /* Syscall / OS */
    Q_TIME          = 0xE8,   /* dest = time_ms()              */
    Q_ENV_GET       = 0xE9,   /* dest = getenv(src1)           */
    Q_SLEEP         = 0xEA,   /* sleep(src1 ms)                */
