...
static const builtin_map_t builtins[] = {
    ...
    /* Network */
    {"net_listen",    BUILTIN_NET_LISTEN},
    {"nghe_mạng",     BUILTIN_NET_LISTEN},
    {"net_accept",    BUILTIN_NET_ACCEPT},
    {"chấp_kết_nối",  BUILTIN_NET_ACCEPT},
    {"net_recv",      BUILTIN_NET_RECV_STR},
    {"nhận_mạng",     BUILTIN_NET_RECV_STR},
    {"net_send",      BUILTIN_NET_SEND_STR},
    {"gửi_mạng",      BUILTIN_NET_SEND_STR},
    {"net_close",     BUILTIN_NET_CLOSE},
    {"đóng_kết_nối",  BUILTIN_NET_CLOSE},
    /* Extended network */
    {"net_send_all",  BUILTIN_NET_SEND_ALL},
    {"gửi_hết_mạng",  BUILTIN_NET_SEND_ALL},
    {"net_set_nodelay", BUILTIN_NET_SET_NODELAY},
    /* Extended string */
    {"str_find",      BUILTIN_STR_FIND},
    {"tìm_chuỗi",     BUILTIN_STR_FIND},
    {"str_slice",     BUILTIN_STR_SLICE},
    {"cắt_chuỗi",     BUILTIN_STR_SLICE},
    {"str_starts_with", BUILTIN_STR_STARTS},
    {"bắt_đầu_bằng",  BUILTIN_STR_STARTS},
    {"str_contains",  BUILTIN_STR_CONTAINS},
    {"chứa_chuỗi",    BUILTIN_STR_CONTAINS},
    {"str_split",     BUILTIN_STR_SPLIT},
    {"tách_chuỗi",     BUILTIN_STR_SPLIT},
    {"str_upper",     BUILTIN_STR_UPPER},
    {"viết_hoa",       BUILTIN_STR_UPPER},
    {"str_lower",     BUILTIN_STR_LOWER},
    {"viết_thường",    BUILTIN_STR_LOWER},
    {"str_replace",   BUILTIN_STR_REPLACE},
    {"thay_chuỗi",    BUILTIN_STR_REPLACE},
    {"str_trim",      BUILTIN_STR_TRIM},
    {"cắt_trắng",     BUILTIN_STR_TRIM},
    {"str_alloc",     BUILTIN_STR_ALLOC},
    {"cấp_chuỗi",     BUILTIN_STR_ALLOC},
    {"sprintf",       BUILTIN_SPRINTF},
    {"định_dạng",      BUILTIN_SPRINTF},
    /* Type conversion / Logic */
    {"logical_not",   BUILTIN_NOT},
    {"not_op",        BUILTIN_NOT},
    {"phủ_định",       BUILTIN_NOT},
    {"i_to_f",        BUILTIN_I_TO_F},
    {"int_to_float",  BUILTIN_I_TO_F},
    {"số_sang_thực",   BUILTIN_I_TO_F},
    {"f_to_i",        BUILTIN_F_TO_I},
    {"float_to_int",  BUILTIN_F_TO_I},
    {"thực_sang_số",   BUILTIN_F_TO_I},
    {"f_to_str",      BUILTIN_F_TO_STR},
    {"float_to_str",  BUILTIN_F_TO_STR},
    {"thực_sang_chuỗi", BUILTIN_F_TO_STR},
    {"str_to_f",      BUILTIN_STR_TO_F},
    {"str_to_float",  BUILTIN_STR_TO_F},
    {"chuỗi_sang_thực", BUILTIN_STR_TO_F},
    /* Syscall / OS */
    {"time_ms",       BUILTIN_TIME},
    {"thời_gian",      BUILTIN_TIME},
    {"env_get",       BUILTIN_ENV_GET},
    {"lấy_biến_mt",   BUILTIN_ENV_GET},
    {"sleep_ms",      BUILTIN_SLEEP},
    {"ngủ",           BUILTIN_SLEEP},
    {"addr",          BUILTIN_ADDR},
    {"địa_chỉ",       BUILTIN_ADDR},
    {"deref",         BUILTIN_DEREF},
    {"giải_tham_chiếu", BUILTIN_DEREF},
    {NULL, 0}
};
...
static ast_node_t *parse_primary(vir_parser_t *p)
{
    ...
    case TOK_IDENT: {
        advance(p);
        if (strcmp(t->str.buf, "new") == 0 && check(p, TOK_IDENT)) {
            const vir_token_t *type_tok = advance(p);
            if (!check(p, TOK_LPAREN)) {
                parse_error(p, "expected '(' after record type in new expression");
                return NULL;
            }
            advance(p);  /* consume '(' */

            ast_node_t *rl = ast_new(AST_RECORD_LITERAL);
            strncpy(rl->name, type_tok->str.buf, AST_NAME_LEN - 1);
            rl->line = t->line;

            if (!check(p, TOK_RPAREN)) {
                ast_node_t *arg = parse_expr(p);
                if (arg) ast_add_child(rl, arg);
            ...
            expect(p, TOK_RPAREN, "expected ')' after record constructor arguments");
            return rl;
        }
...
static ast_node_t *parse_record_def(vir_parser_t *p)
{
    ...
    if (match(p, TOK_LPAREN)) {
        ...
        expect(p, TOK_RPAREN, "expected ')' after record fields");
...
static ast_node_t *parse_statement(vir_parser_t *p)
{
    ...
    case TOK_IDENT: {
        ...
        if (check(p, TOK_ASSIGN)) {
            ...
            match(p, TOK_SEMICOLON);  /* optional ';' after assignment */
            return assign;
        }
        if (check(p, TOK_DOT)) {
            ...
            /* Not field assign - back up to before IDENT, '.' and FIELD */
            p->pos -= 3;  /* back before ident, dot, and field */
