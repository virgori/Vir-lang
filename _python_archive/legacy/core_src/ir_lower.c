...
int lower_expr(lower_ctx_t *ctx, const ast_node_t *expr)
{
    ...
    case AST_IDENTIFIER: {
        ...
        if (scope < 0) {
            for (uint32_t i = 0; i < ctx->enum_type_count; i++) {
                int64_t val = enum_lookup_variant(&ctx->enum_types[i], expr->name);
                if (val >= 0) {
                    uint32_t r = fresh_vreg(ctx);
    ...
    case AST_LITERAL_FLOAT: {
        ...
        uint32_t r = fresh_vreg(ctx);
        double d = expr->float_val;
        int64_t bits;
        memcpy(&bits, &d, sizeof(bits));
        emit(ctx, q_instr(Q_LOAD, q_vreg(r), q_imm(bits), q_none()));
        return (int)r;
    }
    ...
    case AST_BUILTIN_CALL: {
        ...
        case BUILTIN_ARG_COUNT:
            ...
            break;
        case BUILTIN_NET_LISTEN:
            if (a0 >= 0) emit(ctx, q_instr(Q_NET_LISTEN, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_NET_ACCEPT:
            if (a0 >= 0) emit(ctx, q_instr(Q_NET_ACCEPT, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_NET_RECV_STR:
            if (a0 >= 0) emit(ctx, q_instr(Q_NET_RECV_STR, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_NET_SEND_STR:
            if (a0 >= 0 && a1 >= 0)
                emit(ctx, q_instr(Q_NET_SEND_STR, q_none(), q_vreg((uint32_t)a0), q_vreg((uint32_t)a1)));
            emit(ctx, q_instr(Q_LOAD, q_vreg(rd), q_imm(0), q_none()));
            break;
        case BUILTIN_NET_CLOSE:
            if (a0 >= 0) emit(ctx, q_instr(Q_NET_CLOSE, q_none(), q_vreg((uint32_t)a0), q_none()));
            emit(ctx, q_instr(Q_LOAD, q_vreg(rd), q_imm(0), q_none()));
            break;
        /* Extended network */
        case BUILTIN_NET_SEND_ALL:
            if (a0 >= 0 && a1 >= 0)
                emit(ctx, q_instr(Q_NET_SEND_ALL, q_vreg(rd), q_vreg((uint32_t)a0), q_vreg((uint32_t)a1)));
            break;
        case BUILTIN_NET_SET_NODELAY:
            if (a0 >= 0 && a1 >= 0)
                emit(ctx, q_instr(Q_NET_SET_NODELAY, q_none(), q_vreg((uint32_t)a0), q_vreg((uint32_t)a1)));
            emit(ctx, q_instr(Q_LOAD, q_vreg(rd), q_imm(0), q_none()));
            break;
        /* Extended string */
        case BUILTIN_STR_FIND:
            if (a0 >= 0 && a1 >= 0)
                emit(ctx, q_instr(Q_STR_FIND, q_vreg(rd), q_vreg((uint32_t)a0), q_vreg((uint32_t)a1)));
            break;
        case BUILTIN_STR_SLICE:
            if (a0 >= 0 && a1 >= 0 && a2 >= 0)
                emit(ctx, q_instr(Q_STR_SLICE, q_vreg((uint32_t)a2), q_vreg((uint32_t)a0), q_vreg((uint32_t)a1)));
            else if (a0 >= 0 && a1 >= 0)
                emit(ctx, q_instr(Q_STR_SLICE, q_vreg(rd), q_vreg((uint32_t)a0), q_vreg((uint32_t)a1)));
            break;
        case BUILTIN_STR_STARTS:
            if (a0 >= 0 && a1 >= 0)
                emit(ctx, q_instr(Q_STR_STARTS, q_vreg(rd), q_vreg((uint32_t)a0), q_vreg((uint32_t)a1)));
            break;
        case BUILTIN_STR_CONTAINS:
            if (a0 >= 0 && a1 >= 0)
                emit(ctx, q_instr(Q_STR_CONTAINS, q_vreg(rd), q_vreg((uint32_t)a0), q_vreg((uint32_t)a1)));
            break;
        case BUILTIN_STR_SPLIT:
            if (a0 >= 0 && a1 >= 0)
                emit(ctx, q_instr(Q_STR_SPLIT, q_vreg(rd), q_vreg((uint32_t)a0), q_vreg((uint32_t)a1)));
            break;
        case BUILTIN_STR_UPPER:
            if (a0 >= 0) emit(ctx, q_instr(Q_STR_UPPER, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_STR_LOWER:
            if (a0 >= 0) emit(ctx, q_instr(Q_STR_LOWER, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_STR_REPLACE:
            if (a0 >= 0 && a1 >= 0 && a2 >= 0)
                emit(ctx, q_instr(Q_STR_REPLACE, q_vreg((uint32_t)a2), q_vreg((uint32_t)a0), q_vreg((uint32_t)a1)));
            else
                emit(ctx, q_instr(Q_LOAD, q_vreg(rd), q_imm(0), q_none()));
            break;
        case BUILTIN_STR_TRIM:
            if (a0 >= 0) emit(ctx, q_instr(Q_STR_TRIM, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_STR_ALLOC:
            if (a0 >= 0) emit(ctx, q_instr(Q_STR_ALLOC, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_SPRINTF:
            if (a0 >= 0 && a1 >= 0)
                emit(ctx, q_instr(Q_SPRINTF, q_vreg(rd), q_vreg((uint32_t)a0), q_vreg((uint32_t)a1)));
            break;
        /* Type conversion / Logic */
        case BUILTIN_NOT:
            if (a0 >= 0) emit(ctx, q_instr(Q_NOT, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_I_TO_F:
            if (a0 >= 0) emit(ctx, q_instr(Q_I_TO_F, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_F_TO_I:
            if (a0 >= 0) emit(ctx, q_instr(Q_F_TO_I, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_F_TO_STR:
            if (a0 >= 0) emit(ctx, q_instr(Q_F_TO_STR, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_STR_TO_F:
            if (a0 >= 0) emit(ctx, q_instr(Q_STR_TO_F, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        /* Syscall / OS */
        case BUILTIN_TIME:
            emit(ctx, q_instr(Q_TIME, q_vreg(rd), q_none(), q_none()));
            break;
        case BUILTIN_ENV_GET:
            if (a0 >= 0) emit(ctx, q_instr(Q_ENV_GET, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_SLEEP:
            if (a0 >= 0) emit(ctx, q_instr(Q_SLEEP, q_none(), q_vreg((uint32_t)a0), q_none()));
            emit(ctx, q_instr(Q_LOAD, q_vreg(rd), q_imm(0), q_none()));
            break;
        case BUILTIN_ADDR:
            if (a0 >= 0) emit(ctx, q_instr(Q_MOVE, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
        case BUILTIN_DEREF:
            if (a0 >= 0) emit(ctx, q_instr(Q_MOVE, q_vreg(rd), q_vreg((uint32_t)a0), q_none()));
            break;
    ...
    case AST_RECORD_LITERAL: {
        ...
        for (uint32_t i = 0; i < expr->child_count; i++) {
            ...
            if (!child) continue;
            const char *field_name = child->name2;
            if (!field_name[0]) {
                if (i >= rt->field_count) {
                    lower_error(ctx, "too many record constructor arguments");
                    continue;
                }
                field_name = rt->fields[i].name;
            }
            int field_off = record_field_offset(rt, field_name);
            if (field_off < 0) {
                char buf[128];
                snprintf(buf, sizeof(buf), "unknown field: %s", field_name);
                lower_error(ctx, buf);
                continue;
            }
...
int lower_stmt(lower_ctx_t *ctx, const ast_node_t *stmt)
{
    ...
    case AST_ASSIGN: {
        ...
        if (scope < 0) {
            /* v1.2: implicit declaration on first assignment */
            idx = fresh_vreg(ctx);
            sym_define(&ctx->symbols, stmt->name, idx, VIR_TYPE_I64);
            scope = 0;
        }
        int val = lower_expr(ctx, stmt->children[0]);
        if (val < 0) return -1;
...
int lower_func_def(lower_ctx_t *ctx, const ast_node_t *func_def)
{
    ...
    q_vreg_alloc_reset(&ctx->vreg_alloc);
    ...
    /* Restore parent scope and vreg allocator */
