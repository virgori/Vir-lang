...
vm_status_t vm_step(vm_state_t *vm, const q_instruction_t *instr)
{
    ...
    case Q_STR_SLICE: {
        /* dest = src1[src2_start .. (next instr carries end via Q_LOAD trick)
         * Encoding: src1=str, src2=start  + end stored as 3rd arg not available,
         * so we use: src1=str_ptr, src2=start, dest carries end via Q_LOAD_WORD hack.
         * Simpler: use OPERAND layout — src1=str, src2=packed(start<<32|end) */
        const char *s = (const char *)(intptr_t)operand_value(vm, &instr->src1);
        int64_t packed = operand_value(vm, &instr->src2);
        int64_t start = (packed >> 32) & 0xFFFFFFFFLL;
        int64_t end   = packed & 0xFFFFFFFFLL;
        if (!s) { set_dest(vm, &instr->dest, 0); break; }
        int64_t slen = (int64_t)strlen(s);
        if (start < 0) start = 0;
        if (end > slen) end = slen;
        if (start >= end) {
            set_dest(vm, &instr->dest, vm_add_rt_string(vm, ""));
            break;
        }
    ...
    case Q_STR_FIND: {
        /* dest = first occurrence of src2 in src1, or -1 */
        const char *haystack = (const char *)(intptr_t)operand_value(vm, &instr->src1);
        const char *needle   = (const char *)(intptr_t)operand_value(vm, &instr->src2);
        if (!haystack || !needle) { set_dest(vm, &instr->dest, -1); break; }
        const char *found = strstr(haystack, needle);
        set_dest(vm, &instr->dest, found ? (int64_t)(found - haystack) : -1LL);
        break;
    }

    case Q_NOT: {
        int64_t v = operand_value(vm, &instr->src1);
        set_dest(vm, &instr->dest, v == 0 ? 1 : 0);
        break;
    }
    ...
    /* ── Network I/O ───────────────────────────────────── */
    case Q_NET_LISTEN: {
        int64_t port = operand_value(vm, &instr->src1);
        int slot = vir_net_tcp_listen("0.0.0.0", (int)port);
        set_dest(vm, &instr->dest, (int64_t)slot);
        break;
    }
    case Q_NET_ACCEPT: {
        int64_t srv = operand_value(vm, &instr->src1);
        int client = vir_net_tcp_accept((int)srv);
        set_dest(vm, &instr->dest, (int64_t)client);
        break;
    }
    case Q_NET_RECV_STR: {
        int64_t slot = operand_value(vm, &instr->src1);
        char *buf = (char *)malloc(65536);
        if (!buf) { set_dest(vm, &instr->dest, 0); break; }
        int n = vir_net_recv((int)slot, buf, 65535);
        if (n < 0) n = 0;
        buf[n] = '\0';
        set_dest(vm, &instr->dest, vm_add_rt_string(vm, buf));
        break;
    ...
    case Q_NET_SEND_STR: {
        int64_t slot = operand_value(vm, &instr->src1);
        const char *s = (const char *)(intptr_t)operand_value(vm, &instr->src2);
        if (s) vir_net_send((int)slot, s, (int)strlen(s));
        break;
    }
    case Q_NET_CLOSE: {
        int64_t slot = operand_value(vm, &instr->src1);
        vir_net_close((int)slot);
        break;
    }

    /* ── Extended Network ──────────────────────────────── */
    case Q_NET_SEND_ALL: {
        /* Loop-write until all bytes sent */
        int64_t slot = operand_value(vm, &instr->src1);
        const char *s = (const char *)(intptr_t)operand_value(vm, &instr->src2);
        if (s) {
            ...
            int sent = 0;
            while (sent < total) {
                int n = vir_net_send((int)slot, s + sent, total - sent);
                if (n <= 0) break;
                sent += n;
            }
            set_dest(vm, &instr->dest, (int64_t)sent);
        } else {
            set_dest(vm, &instr->dest, 0);
        }
        break;
    }
    case Q_NET_SET_NODELAY: {
        /* setsockopt TCP_NODELAY */
        int64_t slot = operand_value(vm, &instr->src1);
        int64_t flag = operand_value(vm, &instr->src2);
        int val = (int)flag;
        /* slot is an index in net_runtime's fd table;
         * for direct control we access the fd.  For now use slot as fd. */
        (void)setsockopt((int)slot, IPPROTO_TCP, TCP_NODELAY, &val, sizeof(val));
        break;
    }

    /* ── Extended String operations ────────────────────── */
    case Q_STR_STARTS: {
        /* dest = 1 if s starts with prefix, else 0 */
        const char *s = (const char *)(intptr_t)operand_value(vm, &instr->src1);
        const char *pfx = (const char *)(intptr_t)operand_value(vm, &instr->src2);
        if (s && pfx) {
            size_t plen = strlen(pfx);
            set_dest(vm, &instr->dest, strncmp(s, pfx, plen) == 0 ? 1 : 0);
        } else {
            set_dest(vm, &instr->dest, 0);
        }
        break;
    ...
    case Q_STR_CONTAINS: {
        /* dest = 1 if s contains sub, else 0 */
        const char *s = (const char *)(intptr_t)operand_value(vm, &instr->src1);
        const char *sub = (const char *)(intptr_t)operand_value(vm, &instr->src2);
        set_dest(vm, &instr->dest, (s && sub && strstr(s, sub)) ? 1 : 0);
        break;
    }
    case Q_STR_SPLIT: {
        /* dest = array of strings split by delimiter */
        const char *s = (const char *)(intptr_t)operand_value(vm, &instr->src1);
        const char *delim = (const char *)(intptr_t)operand_value(vm, &instr->src2);
        ...
        if (s && delim && delim[0]) {
            ...
            while (p) {
                ...
                memcpy(tok, p, tlen);
                tok[tlen] = '\0';
                vm_array_push(vm, arr, vm_add_rt_string(vm, tok));
                if (found) {
                    p = found + dlen;
                } else {
                    break;
    ...
    case Q_STR_REPLACE: {
        /* dest = str_replace(s, old, new) — src1=s, src2=old, dest has new_str ptr */
        const char *s = (const char *)(intptr_t)operand_value(vm, &instr->src1);
        const char *old_sub = (const char *)(intptr_t)operand_value(vm, &instr->src2);
        const char *new_sub = (const char *)(intptr_t)operand_value(vm, &instr->dest);
        if (s && old_sub && old_sub[0] && new_sub) {
            size_t slen = strlen(s), olen = strlen(old_sub), nlen = strlen(new_sub);
            /* Count occurrences */
            size_t count = 0;
            const char *p = s;
            while ((p = strstr(p, old_sub)) != NULL) { count++; p += olen; }
            size_t rlen = slen + count * (nlen - olen);
            char *buf = (char *)malloc(rlen + 1);
            char *w = buf;
            p = s;
            while (*p) {
                if (strncmp(p, old_sub, olen) == 0) {
                    memcpy(w, new_sub, nlen);
                    w += nlen;
                    p += olen;
                } else {
                    *w++ = *p++;
            ...
            *w = '\0';
            ...
            if (s) {
                char *dup = strdup(s);
                set_dest(vm, &instr->dest, vm_add_rt_string(vm, dup));
            } else {
                set_dest(vm, &instr->dest, 0);
            }
        }
    ...
    case Q_STR_ALLOC: {
        /* Allocate a zeroed string buffer of given size */
        int64_t sz = operand_value(vm, &instr->src1);
        if (sz <= 0) sz = 1;
    ...
    case Q_SPRINTF: {
        /* dest = sprintf(fmt, a, b) — simpler: fmt + one int arg */
        const char *fmt = (const char *)(intptr_t)operand_value(vm, &instr->src1);
        int64_t arg = operand_value(vm, &instr->src2);
        if (fmt) {
            char *buf = (char *)malloc(1024);
            snprintf(buf, 1024, fmt, (long long)arg);
            set_dest(vm, &instr->dest, vm_add_rt_string(vm, buf));
        } else {
            set_dest(vm, &instr->dest, 0);
        }
    ...
    case Q_I_TO_F: {
        /* Convert int64 → double, store as bit-pattern */
        int64_t v = operand_value(vm, &instr->src1);
        double d = (double)v;
        int64_t bits;
        memcpy(&bits, &d, sizeof(bits));
        set_dest(vm, &instr->dest, bits);
        break;
    }
    case Q_F_TO_I: {
        /* Convert double (bit-pattern) → int64 */
        int64_t bits = operand_value(vm, &instr->src1);
        double d;
        memcpy(&d, &bits, sizeof(d));
        set_dest(vm, &instr->dest, (int64_t)d);
        break;
    }
    case Q_F_TO_STR: {
        /* Convert double (bit-pattern) → string */
        int64_t bits = operand_value(vm, &instr->src1);
        double d;
        memcpy(&d, &bits, sizeof(d));
    ...
    case Q_STR_TO_F: {
        /* Convert string → double (bit-pattern) */
        const char *s = (const char *)(intptr_t)operand_value(vm, &instr->src1);
    ...
    /* ── Syscall / OS ──────────────────────────────────── */
    case Q_TIME: {
        /* dest = current time in milliseconds */
        struct timeval tv;
        gettimeofday(&tv, NULL);
        int64_t ms = (int64_t)tv.tv_sec * 1000 + (int64_t)tv.tv_usec / 1000;
        set_dest(vm, &instr->dest, ms);
        break;
    }
    case Q_ENV_GET: {
        const char *name = (const char *)(intptr_t)operand_value(vm, &instr->src1);
        if (name) {
    ...
    case Q_SLEEP: {
        int64_t ms = operand_value(vm, &instr->src1);
